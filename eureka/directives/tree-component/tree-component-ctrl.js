(function () {
    ...
    function TreeComponentCtrl(PhenotypeService, NgTableParams, TreeService) {
        ...
        TreeService.getTreeRoot().then(function (data) {
            // NG Table
            vm.tableParams = new NgTableParams({page: 1, count: data.length}, {dataset: data});
        }, displayError);

        PhenotypeService.getPhenotypeRoot().then(function (data) {
            // NG Table
            vm.tableParamsUser = new NgTableParams({page: 1, count: data.length}, {dataset: data});
        }, displayError);
        ...
        vm.selectNode = function (node) {
            ...
            if (hasChildren !== false) {
                TreeService.getTreeNode(currentNode.key).then(function (data) {
                    ...
                    vm.tableParams = new NgTableParams({page: 1, count: data.children.length}, {dataset: data.children});
                }, displayError);
            }
        };
        ...
        vm.setBreadCrumbs = function (node) {
            ...
            TreeService.getTreeNode(currentNode.key).then(function (data) {
                ...
                vm.tableParams = new NgTableParams({page: 1, count: returnedData.length}, {dataset: returnedData});
